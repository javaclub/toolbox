...
import com.github.javaclub.toolbox.util.GZipUtil;
...
public final class DownloadUtil {
	...
	private static final String[] USER_AGENTS = new String[] {
		...
		"Mozilla/5.0 (Linux; U; Android 2.3.3; zh-cn; HW-HUAWEI_C8800/C8800V100R001C92B818; 480*800; CTC/2.0) AppleWebKit/533.1 Mobile Safari/533.1",
		"Mozilla/4.0 (compatible; MSIE 8.0; Windows NT 6.1; Trident/4.0; qdesk 2.3.1186.202; QQDownload 715; SLCC2; .NET CLR 2.0.50727; .NET CLR 3.5.30729; .NET CLR 3.0.30729; Media Center PC 6.0; 4399Box.1225; 4399Box.909; 4399Box.1225; 360SE)"
	};
    ...
    public static HttpResponse download(String url, String referer, long ifModifiedSince, String defaultEncoding) {
        ...
        while ( retry > 0 ) {
            ...
            try {
                ...
                hc.addRequestProperty("User-Agent", mockedUserAgent());
                hc.addRequestProperty("Accept-Encoding", ACCEPT_ENCODING);
                if ( null != referer ) hc.addRequestProperty("Referer", referer);
                ...
                if (responseCode == HttpResponse.SC_OK) {
                    ...
                    if (HttpResponse.isText(contentType)) {
                        ...
                        if ( null == contentEncoding ) contentEncoding = defaultEncoding;
                        return HttpResponse.ok(url, contentType, contentEncoding, data);
                    }
     ...
     * "start" parameter.
     */
    private static boolean startsFrom(byte[] source, byte[] target, int start) {
        if(source==null || target==null)
            return false;
    ...
    private static String mockedUserAgent() {
    	if(null != USER_AGENTS && USER_AGENTS.length > 0) {
    		arrIdx++;
    		if(arrIdx == USER_AGENTS.length) arrIdx = 0;
