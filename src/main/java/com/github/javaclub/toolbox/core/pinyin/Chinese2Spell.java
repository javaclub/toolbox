 ...
 * 都查不到，所以只好自己做一个汉字转拼音的API，该API除可以处理两万以上的汉字
 * 而且可以返回多音字的读音
 * JDK版本: 1.4
 */
public class Chinese2Spell {
	
	private static final Log log = LogFactory.getLog(Chinese2Spell.class);
	...
	public static void init116() {
		spellPut("161-163", "。");
		spellPut("161-175", "’");
	}

	/**
	 * 获得单个汉字的Ascii，并甄1�7"-"连接成一个字符串
	 * 
	 * @param cn char 汉字字符
	 ...
	 * 
	 * 否则到SpellMap中查找,如果没有找到拼音,则返回null,如果找到则返回拼韄1�7.
	 */
	...
	public static String getFullSpell(String cnStr) throws UnsupportedEncodingException
	{
		...
		for (int i = 0, Len = chars.length; i < Len; i++) {
			...
			if (ascii.length() == 0) { // 取ascii时出锄1�7
	...
	/**
	 * 获取汉语字符串的声母组合，每个汉字取拼音的第一个字符组成的字符串
	 * @param cnStr 汉字的字符串
	...
	public static String getFirstSpell(String cnStr) throws UnsupportedEncodingException
	{
		...
		for (int i = 0, Len = chars.length; i < Len; i++) {
			...
			if (ascii.length()==0||ascii.length() == 1) { // 取ascii时出锄1�7
				retuBuf.append(chars[i]);
			} else {
	...
	public static void main(String[] args)throws UnsupportedEncodingException {
		...
		String str = null;
		str = "囧是";
		System.out.println("Spell=" + Chinese2Spell.getFullSpell(str));
		System.out.println("Spell=" + Chinese2Spell.getFirstSpell(str));
		Date end = new Date();
